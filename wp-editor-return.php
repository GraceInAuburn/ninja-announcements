...
class WP_Editor {
	...
	function editor( $content, $editor_id, $settings = array(), $media_buttons = true ) {
		...
		$set = wp_parse_args( $settings,  array(
			...
			'wp_buttons_css' => WP_PLUGIN_URL . '/ninja-forms/css/editor-buttons.css', // styles for both visual and HTML editors buttons
		...
		$editor_output .= '<div id="wp-' . $id . '-wrap" class="wp-editor-wrap">';

		if ( !is_admin() && !empty($set['wp_buttons_css']) )
			$editor_output .= '<style type="text/css" scoped="scoped"> @import url("' . $set['wp_buttons_css'] . '"); ' . "</style>\n";
		...
		$the_editor = apply_filters('the_editor', '<div id="wp-' . $id . '-editor-container" class="wp-editor-container"><textarea class="' . $set['editor_class'] . '" rows="' . $rows . '" cols="40" name="' . $name . '" id="' . $id . '">%s</textarea></div>');
		$the_editor_content = apply_filters('the_editor_content', $content);

		$editor_output .= apply_filters( 'the_editor_toolbar', $toolbar );
		$editor_output .= sprintf($the_editor, $the_editor_content);
		$editor_output .= "\n</div>\n\n";
